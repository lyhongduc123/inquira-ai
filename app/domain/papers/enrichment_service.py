...
class PaperEnrichmentService:
    ...
    async def batch_enrich_papers_with_authors_institutions(
        self,
        papers_data: List[tuple[int, PaperEnrichedDTO]]
    ) -> int:
        """
        Batch enrich multiple papers with authors and institutions.
        
        Args:
            papers_data: List of tuples (paper_id, paper_dto)
            
        Returns:
            Number of successfully enriched papers
        """
        success_count = 0
        
        for paper_id, paper_dto in papers_data:
            try:
                result = await self.enrich_paper_with_authors_institutions(
                    paper_id=paper_id,
                    paper_dto=paper_dto
                )
                if result:
                ...
                logger.error(f"Error batch enriching paper {paper_id}: {e}")
                continue
        
        logger.info(f"Batch enriched {success_count}/{len(papers_data)} papers with authors")
