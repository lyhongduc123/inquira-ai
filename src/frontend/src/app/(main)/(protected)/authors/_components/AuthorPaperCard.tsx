...
export function AuthorPaperCard({
  ...
  const formatAuthors = (authorsArr: AuthorMetadataDTO[]) => {
    ...
    const renderName = (
      ...
      return (
        <span
          key={`${name}-${index}`}
          className={cn(isMatch && "font-bold text-special")}
        >
          {name}
          {hasComma && ", "}
        </span>
    ...
    if (authorsArr.length <= 4) {
      return (
        <>
          {authorsArr.map((a, i) =>
            renderName(a.name, i, i < authorsArr.length - 1),
          )}
        </>
      );
    }
    ...
    const matchIndex = currentAuthorName
      ? authorsArr.findIndex(
          (a) => a.name.toLowerCase() === currentAuthorName.toLowerCase(),
        )
      : -1;
  ...
  return (
    <Card
      className={cn(
        "group relative gap-2 p-3 pl-4 transition hover:bg-card/40 hover:border-primary cursor-pointer min-w-0",
    ...
    >
      <Box
        className={cn(
          "absolute left-2 text-xs rounded-full font-semibold size-6 justify-center items-center flex",
          "bg-gray-400 text-white",
          journal?.sjrBestQuartile === "Q1" &&
            "bg-green-500 text-white",
          journal?.sjrBestQuartile === "Q2" &&
            "bg-yellow-500 text-white",
          journal?.sjrBestQuartile === "Q3" &&
            "bg-orange-500 text-white",
          journal?.sjrBestQuartile === "Q4" &&
            "bg-red-400 text-white",
          !journal && conference?.rank === "A*" &&
            "bg-green-500 text-white",
          !journal && conference?.rank === "A" &&
            "bg-yellow-500 text-white",
          !journal && conference?.rank === "B" &&
            "bg-orange-500 text-white",
          !journal && conference?.rank === "C" &&
            "bg-red-400 text-white",
        )}
        title={journal?.title || conference?.title || ""}
      >
        {journal?.sjrBestQuartile || conference?.rank || "N/A"}
      </Box>
            ...
            {authors && (
              <HStack className="flex-wrap items-center gap-3 text-sm text-accent">
                <HStack className="min-w-0 items-center gap-1">
                  ...
                  <TypographyP size="xs" className="line-clamp-1">
